...
if __name__ == '__main__':
    ...
    for metric in metrics:
        ...
        df_metric['experiment_type'] = df_metric.experiment_type.str.extract(r'(\d+)').astype(int)
        df_metric = df_metric.groupby(["experiment_type", "algorithm"], as_index=False).agg({"score": np.mean})
        ax = sns.lineplot(data=df_metric, x='experiment_type',y='score',hue='algorithm')
        ax.set_xlabel('window size $\omega$')
        metric_label = metric
        if metric == 'total_running_time':
            metric_label = 'Total running time in [s]'
        ax.set_ylabel(metric_label)
        #plt.show()
        plt.savefig(f'../results/{metric}_window_size.png')
        plt.close()
